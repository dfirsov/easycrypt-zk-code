...
apply has_E'. auto.
split. smt. progress.
have ->: (weight (drestrict X M)) = mu X M. smt.
auto.
qed.
...
have ->: (fun (x0 : 'a) => mass X x0) = (fun (x0 : 'a) => mu1 X x0). apply fun_ext. smt.
apply isdistr_mu1. 
split.
smt.
progress. 
...
rewrite muK. 
   apply isdistr_mu1. simplify. auto. auto. smt.
smt.
qed.
