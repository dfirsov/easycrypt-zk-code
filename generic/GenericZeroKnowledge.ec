...
clone import HybridArgumentWithParameter as Hyb with type input <- unit,
  ...
  op zk_function : statement -> real.
  axiom statistical_zk: exists (HP <: HonestProver) 
                               (Sim <: Simulator), 
   forall s w &m 
         (V <: RewMaliciousVerifier)
         (D <: ZKDistinguisher),
    zk_relation s w =>
     `|Pr[ ZKReal(HP,V,D).run(s,w)@&m: res]
         - Pr[ZKIdeal(Sim,V,D).run(s,w)@&m: res]|
       <= zk_function s.  
  end Statement.
