...
module  Simulator(S : Simulator1)(V : RewMaliciousVerifier)  = {
  ...
  proc simulate(statement : statement) :
   ...
   declare axiom D_guess_prop : equiv[ D.guess ~ D.guess : ={glob V, arg} ==> ={res} ].
    ...
    lemma computational_statisticalVpoly_zk stat wit p0 negl &m:
...
local lemma lll &m deltoid : 
   (forall &n,
   Pr[ZKIdeal(Sim, V, Dstar).run(s, w) @ &n : res]
...
have -> : Pr[Amem.run1() @ &n : res] = Pr[ZKIdeal(Sim, V, Dstar).run(s, w) @ &n : res].
byequiv. proc.         
...
lemma zk_seq &m deltoid : 
   ...
   Pr[ZKIdeal(SimAmp(Sim), V, D).run(s, w) @ &m : res]
        - Pr[ZKRealAmp(P, V, D).run(s, w) @ &m : res]
          < n%r * deltoid.
move => zk_ass.
have -> : Pr[ZKIdeal(SimAmp(Sim), V, D).run(s, w) @ &m : res]
 = Pr[Ln(Ob,A).main() @ &m : res].
byequiv (_:   
...
while (={i, glob V, glob Sim} /\ summary0{1} = summary{2} /\ statement0{1} = s ).
