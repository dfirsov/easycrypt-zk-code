...
have -> : size x0 = size (fap prm s{hr}.`2). smt(supp_djoinmap).
have -> : size (fap prm s{hr}.`2) = size (s{hr}.`2). smt. smt.
have ->: size (unzip2 (take s{hr}.`1 (ip (prj (permute_wit prm w{hr})) x0)))
 = size (take s{hr}.`1 (ip (prj (permute_wit prm w{hr})) x0)). smt.
have : s{hr}.`1 <= size (ip (prj (permute_wit prm w{hr})) x0).
have ->: size (ip (prj (permute_wit prm w{hr})) x0) = size x0.
smt.
have -> : size x0 = size (fap prm s{hr}.`2). smt(supp_djoinmap).
have -> : size (fap prm s{hr}.`2) = size (s{hr}.`2). smt. timeout 20. smt.
progress. smt.
have ->: 
  (unzip2 (take s{hr}.`1 (ip (prj (permute_wit prm w{hr})) x0)))
  = ((take s{hr}.`1 (unzip2 (ip (prj (permute_wit prm w{hr})) x0)))).
smt.
have ->: 
  (zip (take s{hr}.`1 (ip (prj (permute_wit prm w{hr})) (unzip1 x0)))
     (take s{hr}.`1 (unzip2 (ip (prj (permute_wit prm w{hr})) x0))))
  = take s{hr}.`1  
...
search djoin.
  have :   size (fap prm s{hr}.`2) = size x0 /\ all (fun xy => snd xy \in Com xy.`1) (zip fapg x0).
