...
section.
declare module V : MaliciousVerifier {HonestProver,HP'}.
...
(* one-time simulator  *)
module Sim1_0(V : MaliciousVerifier, D : ZKDistinguisher) = {
...
lemma sim1_main &m pa wa aux:  IsHC (pa,wa) =>
...
module Sim1(V : MaliciousVerifier) : Simulator1 = {
  module ZKP_HP = ZKP(HonestProver,V)
  module ZKP_HP' = ZKP(HP',V)
  proc sinit(p_a : hc_prob) : bool * (hc_com * hc_resp) = {
    var n, g,bb,r;
...
axiom negl2_prop : 0%r <= negl2 < 1%r/4%r.
lemma qqq &m (p : hc_prob) (w : hc_wit) 
 (aux : auxiliary_input) (D <: ZKDistinguisher{HP', HonestProver}) 
     (V <: MaliciousVerifier{D, HP', HonestProver}): 
   islossless D.guess =>
...
call {2} H. simplify. inline   W0(Sim1(V), D).Sim1.run. sp. wp. call (_:true).
call (_:true). simplify. call(_:true).
sim. skip. progress.  smt.

apply (sim1_main V D H _ _ &m p w aux). assumption.
apply negl2_prop.
auto.
qed.
...
module  (Simulator : Simulator)(V : MaliciousVerifier)  = {
  module M = MW.IFB.IM.W(Sim1(V))
  proc simulate(statement : hc_prob, n : int, aux : auxiliary_input) :
    adv_summary = {
       var r;
...
lemma hc_statistical_zk:
        forall (V <: MaliciousVerifier{HonestProver,HP', MW.IFB.IM.W, MW.IFB.DW}) (D <: ZKDistinguisher{HP', HonestProver, MW.IFB.IM.W, V}),
        ...
        let ideal_prob = Pr[ZKIdeal(Simulator, V, D).run(stat, wit, N, ax) @ &m : res] in
          `|real_prob - ideal_prob| <= (2%r * negl + 20%r * negl2) + 2%r * (1%r- (1%r/2%r - negl2) ) ^ N .
simplify. progress.

have ->: 
 `|Pr[ZKReal(HonestProver, V, D).run(stat, wit, ax) @ &m : res] -
  Pr[ZKIdeal(Simulator, V, D).run(stat, wit, N, ax) @ &m : res]|
 = `|Pr[ZKIdeal(Simulator, V, D).run(stat, wit, N, ax) @ &m : res] - Pr[ZKReal(HonestProver, V, D).run(stat, wit, ax) @ &m : res]|. smt.

have ->: Pr[ZKIdeal(Simulator, V, D).run(stat, wit, N, ax) @ &m : res]
...
byequiv (_:  E{2} = fst /\ aux{1} = aux{2} /\ n{1} = ea{2} /\ fevent{2} = false  /\
  statement{1} = Ny{2} /\ witness{1} = w{2} /\
    ={glob HP', glob HonestProver, glob D, glob V, glob MW.IFB.IM.W} ==> _)  ;auto. proc.
inline Iter(Sim1(V), D).WI.run. wp.  sp. simplify.
call (_:true).  simplify. inline Simulator(V).simulate. wp. sp.
...
seq 1 : (true). rnd. auto. rnd. auto. smt. if. inline*. sp. wp. simplify. rnd. 
conseq (_: _ ==> true). progress. apply djoinmap_weight.
apply Com_lossless. wp. rnd. skip.
progress. apply perm_d_lossless.
inline*. wp. simplify. rnd. 
conseq (_: _ ==> true). progress. 
apply djoinmap_weight. apply Com_lossless.
rnd.  conseq (_: _ ==> true). progress. 
apply djoinmap_weight. apply Com_lossless.
wp. simplify. sp. rnd. skip. progress. apply perm_d_lossless.
hoare. rnd. skip. smt. auto. skip. auto.
apply H1.  auto.
apply (qqq &m stat wit ax D V). apply H1. apply H. auto. auto.
have ->: Pr[Sim1(V).run(stat, ax) @ &m : res.`1] = 
 Pr[Sim1_0(V,D).simulate(stat, ax, wit) @ &m : res.`1] .
progress. byequiv (_: _ ==> res{1}.`1 = res{2}.`1);auto. proc.
call {2} H1. simplify. inline   W0(Sim1(V), D).Sim1.run. sp. wp. call (_:true).
call (_:true). simplify. call(_:true).
sim. skip. progress.  smt.
have f : `|Pr[Sim1_0(V, D).simulate(stat, ax, wit) @ &m : res.`1] - 1%r / 2%r|
 <= negl2. 
apply (sim1ass V D). auto. auto. apply negl2_prop. 
smt.
qed.
