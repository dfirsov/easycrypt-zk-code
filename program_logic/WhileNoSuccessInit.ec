...
require WhileSplitInit.
clone import WhileSplitInit as IM with type sbits <- sbits,
                                   type iat <- iat,
                                   type rrt <- rrt,
                                   type irt <- irt.
...
lemma final_zz_ph_le p MyPa i e r :  MyPa r = false => 0 <= e =>
   phoare[ A.run : arg = i ==> !MyPa res ] <= p =>
   phoare [ W(A).whp : arg = (MyPa, i, 1,e,r) ==> !MyPa res ] <= (p ^ (e)).
move => sf ep ph1.
bypr. progress.
have ff : Pr[A.run(i) @ &m : ! MyPa res] <= p. 
byphoare (_: arg = i ==> _). apply ph1. auto. auto.
rewrite H. simplify.
apply (final_zz_le &m p MyPa i e r sf ep ff).
qed.
