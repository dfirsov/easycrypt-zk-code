...
axiom Sim1_init_ll : islossless Sim1.init.
...
have ->: Pr[W0(Sim1,D).run(p,w,aux) @ &m : ! E res.`2]
  ...
  byphoare. 
proc. call D_ll. call Sim1_ll.  admit. auto. auto. auto.
...
have ->: Pr[W0(Sim1,D).run(p,w,aux) @ &m : ! E res.`2]
  = 1%r - Pr[W0(Sim1,D).run(p,w,aux) @ &m :  E res.`2].
  have ->: 1%r = Pr[W0(Sim1,D).run(p,w,aux) @ &m :  true].
  byphoare. proc.  call D_ll. call Sim1_ll. admit. auto. auto. auto. 
  have ->: Pr[W0(Sim1,D).run(p,w,aux) @ &m : true] = Pr[W0(Sim1,D).run(p,w,aux) @ &m : E res.`2]
   + Pr[W0(Sim1,D).run(p,w,aux) @ &m : !E res.`2]. rewrite Pr[mu_split E res.`2]. 
  simplify. smt. smt.
  have : 
...
lemma one_to_many_zki &m p w p0 eps ea zkp aux:
     `| Pr[ W0(Sim1,D).run(p,w,aux) @ &m : E res.`2 /\ res.`1 ]
           / Pr[W0(Sim1,D).run(p,w,aux) @ &m : E res.`2 ] - zkp | <= eps
  => 0  <= ea
  => (forall &m, p0 <= Pr[ Sim1.run(p,aux) @ &m : E res ]) =>
...
apply (zk_final_le &m p w p0 eps ea zkp aux);auto.    
   have qp: phoare[ Sim1.run :  (p,aux) = (arg.`1,arg.`2) ==> E res ] >= p0. bypr.   
    progress. smt. 
    byphoare (_: (p,w,aux) = (arg.`1,arg.`2,arg.`3) ==> _). proc.
    inline*. 
    simplify. 
    seq 2 : (E r). auto.  call qp. call (_:true).  apply Sim1_init_ll.  skip. auto.
    admit.
     auto. auto. auto. auto. 
...
have bf2 : Pr[Iter(Sim1, D).run(fevent, p, w, aux, ea, E) @ &m : ! E res.`2]
  <= (1%r - p0) ^ ea.
  have bf3: forall &m, Pr[Sim1.run(p,  aux) @ &m : !E res] <= 1%r - p0. progress.
    have -> : 1%r = Pr[Sim1.run(p, aux) @ &m0 : true]. byphoare.
apply Sim1_ll. auto. auto. progress.
       have : Pr[Sim1.run(p, aux) @ &m0 : true] 
                - Pr[Sim1.run(p, aux) @ &m0 : !E res] 
                 = Pr[Sim1.run(p, aux) @ &m0 : E res]. 
    rewrite Pr[mu_split E res]. simplify. smt. 
  smt.
  have ->: Pr[Iter(Sim1, D).run(fevent, p, w, aux, ea, E) @ &m : ! E res.`2] 
     = Pr[ W(Sim1).whpi(E,(p,aux),1,ea,(fevent,witness)) @ &m : ! E res ]. 
   byequiv. proc*.  inline W(Sim1).whpi. inline Iter(Sim1, D).run. sp. wp. inline Iter(Sim1, D).WI.run.
 sp. wp. call {1} D_ll. 
   conseq (_: _==> r1{1} = q{2}). smt. 
call (_: ={glob Sim1}).  sim. call (_:true). skip. auto. auto. auto.
apply (final_zz_lei (Sim1) Sim1_ll _ &m). apply Sim1_rew_ph. smt. auto.  
smt.
smt.
qed.
